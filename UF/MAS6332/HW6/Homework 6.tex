...
\begin{enumerate}
    ...
    \item (15.1.28) Prove that if $V$ and $W$ are affine algebraic sets, then so is $V\times W$ and $k[V\times W]\cong k[V]\otimes_kk[W]$. \begin{proof}
        ...
        We define the map $\Psi\colon k[V]\otimes_kk[W]\to k[V\times W]$ by defining it on generators. %Let $\Psi$ map $\overline{f}\otimes \overline{g}$ to $\overline{f+g}$. We check well definedness: If $s\in \abr{S}$ and $t\in \abr{T}$ then we may view $s,t\in \abr{S\cup T}$ so that $\overline{f+s}\otimes \overline{g+t}\mapsto \overline{f+g+s+t} = \overline{f+g}$ as needed. The map $\Phi$ is injective. If 
    \end{proof}
