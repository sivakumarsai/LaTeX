...
\begin{proposition}\label{prop: eigenspace decomp}
    ...
    \[\text{\textib{$\mathcal M_k(N,\chi)$}} = \{f\in \mathcal M_k(\varGamma_1(N)) : f|_k[\gamma] = \chi(d_\gamma)f \text{ for all } \gamma\in\varGamma_0(N)\},\] where $d_\gamma$ denotes the lower right entry of $\gamma$. Then the following decomposition holds:
    ...
    \[\mathcal S_k(\varGamma_1(N)) = \bigoplus_\chi \text{\textib{$\mathcal S_k(N,\chi)$}},\quad \mathcal E_k(\varGamma_1(N)) = \bigoplus_\chi \text{\textib{$\mathcal E_k(N,\chi)$}}.\]
\end{proposition}
\begin{proof}
    \tbd 
\end{proof}
...
\begin{proof}
    ...
    \begin{definition}
        ...
        The ring $\mathcal R^\ast(N)$ is just the ring of all adjoints of Hecke operators in the Hecke algebra $\mathcal R(N)$.
    \end{definition}
...
Let \textib{$\alpha_\ell$} be given by 
\[\begin{pmatrix}
    \ell & 0 \\ 0 & 1
\end{pmatrix}\] for a positive integer $\ell$. 
...
In this section we focus on simultaneous eigenfunctions of the Hecke operators. \sai{check this}
...
\sai{equation 4.6.18 !!!, some elaboration needed} 
...
\begin{lemma}\label{lem: miyake lem 4.5.11} \sai{bro what is this, i think this needs adjustment due to using $\varGamma_1(N)$}
...
\begin{theorem}\label{thm: miyake thm 4.5.16}
    ...
    \end{enumerate}
    Moreover, if $f(z)$ satisfies the above conditions, then $f|_k[T_n] = t_nf$ for all $n\geq 1$.
\end{theorem}
\begin{proof}
    \tbd
\end{proof}
