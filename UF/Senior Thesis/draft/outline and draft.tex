...
\begin{definition}
    ...
    Define also the \textbf{\textit{weight $k$ }}\textib{Eisenstein space \textup{(}of $\varGamma$\textup{)}} by the quotient of modular forms by cusp forms, $\mathcal E_k(\varGamma) = \mathcal M_k(\varGamma)/\mathcal S_k(\varGamma)$. We briefly discuss this space at the end of \cref{section eisenstein series}.
\end{definition}
...
\begin{lemma}
    ...
    \[\sigma(\varGamma\cap \slz)\sigma^{-1}\cdot\cbr{\pm I} = \cbr{\pm\begin{pmatrix}
        1 & h \\ 0 & 1
    \end{pmatrix}^m : m\in\mathbb Z}\] for some $h>0$.
\end{lemma}
\begin{proof}
    Without loss of generality take $x = \infty$ by taking $\sigma\varGamma\sigma^{-1}$ in place of $\varGamma$. There exists an element  $\gamma = \big(\!\begin{smallmatrix}
    1 & \ell \\ 0 & 1
\end{smallmatrix}\!\big)$ with $\ell\neq 0$ in the stabilizer $\varGamma_\infty$ of $\infty$. Let $\alpha$ is an element of $\glq$ of the form $\big(\!\begin{smallmatrix}
    a & b \\ 0 & a^{-1}
...
\end{pmatrix}\in \varGamma.\] We take as a fact that congruence subgroups are discrete topological groups, so that the previous statement yields a contradiction. It follows then that $\varGamma_\infty\subset\cbr{\pm\big(\!\begin{smallmatrix}
    1 & b \\ 0 & 1
\end{smallmatrix}\!\big) : b\in\mathbb R}$. We obtain the result by observing that $\cbr{\pm\big(\!\begin{smallmatrix}
    1 & b \\ 0 & 1
\end{smallmatrix}\!\big) : b\in\mathbb R}/\cbr{\pm I}\cong \mathbb R$.
\end{proof}
\begin{lemma}
...
\end{lemma}
\begin{proof}
    We may assume that $k$ is even by squaring $f$. Let $x$ be a cusp of $\varGamma$ and suppose that $x$ is rational. Then there exists $\sigma\in\slz$ such that $\sigma(x) = \infty$.
\end{proof}
...
\sai{main lemma?? how do i want to do this}

\newpage\section{Strong Multiplicity One}
\subsection{TEMPORARY bucket}
...
\end{lemma}
\begin{proof}
    Observe that $f((az+b)/d) = \sum_{n=0}^\infty c_ne^{2\pi in(az+b)/d}$. Then by the explicit action of $T(n)$ on elements of $\mathcal M_k(N,\chi)$ (recorded in \sai{insert cref to when this happens yo, it's after lemma 4.5.9}, we have
