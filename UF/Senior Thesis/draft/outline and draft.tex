...
\section*{Acknowledgements}\sai{check this}
\newpage\tableofcontents

\newpage\section*{Introduction}\sai{check this}
\addcontentsline{toc}{section}{Introduction}

\newpage\section{Preliminaries} \pagestyle{body} In this section, we collect the definitions and results needed to define modular forms. \sai{check this}
...
In this section, we define modular forms, provide dimension formulas for the spaces of modular forms appearing throughout, discuss $L$-functions corresponding to modular forms, and provide definitions of Eisenstein series as important examples of modular forms.\sai{check this}
...
From $\abs{q} = e^{-2\pi \Im z/h}$, it follows that $q$ tends to zero as $\Im z$ tends to infinity. We define $f$ to be meromorphic (respectively holomorphic) at $\infty$ if $\tilde f$ has a meromorphic (respectively holomorphic) extension to $q = 0$ (and in the holomorphic case the Laurent series at $q=0$ sums over the nonnegative integers). The Laurent series of $\tilde f$ about $q=0$ is used to obtain a Fourier series expansion of $f$ about $\infty$, given by 
...
\begin{proof}
    ...
    Let $x$ be a cusp of $\varGamma$ and suppose that $x$ is rational. Then there exists $\sigma\in\slz$ such that $\sigma(x) = \infty$. Let $h>0$ be an integer satisfying the result of the previous lemma. Then the Fourier series expansion of $f|_k[\sigma^{-1}]$ is 
...
\begin{proposition}\label{prop: eigenspace decomp}
    ...
    \[\text{\textib{$\mathcal M_k(N,\chi)$}} = \{f\in \mathcal M_k(\varGamma_1(N)) : f|_k[\gamma] = \chi(\gamma)f \text{ for all } \gamma\in\varGamma_0(N)\}.\] Similarly define $\chi$-eigenspaces of $\mathcal S_k(\varGamma_1(N))$. Then the following decompositions hold:
    \[\mathcal M_k(\varGamma_1(N)) = \bigoplus_\chi\mathcal M_k(N,\chi),\quad \mathcal S_k(\varGamma_1(N)) = \bigoplus_\chi \text{\textib{$\mathcal S_k(N,\chi)$}}.\]
\end{proposition}
...
\begin{proof}
    ...
    Let $\phi_1(s) = \phi(s)/\psi(s)$. The function $\phi_1$ satisfies the same assumptions as $\phi$ with $b = 0$, so by repeating the above argument with $\phi_1(s)$ in place of $\phi(s)$, we find that $\phi_1(s)$ is bounded on $F$. It follows that $\abs{\phi(s)} = \mathcal O(\abs{\tau}^b)$ for $\abs{\tau}\to\infty$. 
...
We outline how double cosets act on spaces of modular forms, and using them we define the Hecke operators $\abr{n}$ and $T_n$ on the space $\mathcal M_k(\varGamma_1(N))$. Then we restrict to the space of cusp forms $\mathcal S_k(\varGamma_1(N))$, on which we define the Petersson inner product and find adjoints to the Hecke operators $\abr{p}$ and $T_p$ for $p$ not dividing $N$. By doing so, we deduce that the Hecke operators $\abr{n}$ and $T_n$ are normal operators on $\mathcal S_k(\varGamma_1(N))$ whenever $n$ is coprime to $N$. \sai{check this}
...
with a similar definition for $\mathcal S_k(N,\chi)$.
...
\begin{proof}
    ...
    % \[\text{\textib{$\mathcal M_k(N,\chi)$}} = \{f\in \mathcal M_k(\varGamma_1(N)) : f|_k[\gamma] = \chi(\gamma)f \text{ for all } \gamma\in\varGamma_0(N)\}\] with a similar definition of $\chi$-eigenspaces of $\mathcal S_k(\varGamma_1(N))$ given by replacing $\mathcal M_k(\varGamma_1(N)$ by $\mathcal S_k(\varGamma_1(N)$.
...
\end{proof}
In other words, the diamond operator $\abr{d}$ acts on $\mathcal M_k(\varGamma_1(N)) = \bigoplus_\chi\mathcal M_k(N,\chi)$ by acting on each $\chi$-eigenspace by multiplication by $\chi(d)$.
...
\begin{definition}
    The \textbf{\textit{Hecke algebra}} \textib{$\mathcal R(N)$} is the polynomial ring 
...
\begin{remark}
    In \cite{miyake}, the Hecke algebra is generated by Hecke operators $T(q), T(p,p)$ that are defined differently. The Hecke operators $T(q), T(p,p)$ are related to the ones we have defined by the equalities $T(p) = T_p$ and $T(p,p) = p^{k-2}\abr{p}$.
\end{remark} 
...
\begin{proof}
    ...
    \[\sideset{}{^{\prime\prime}}\sum_{m}t_m m^{-s} = \prod_{p\in P}(1-t_pp^{-s} + \chi(p)p^{k-1-2s})^{-1}\] by a similar argument as in \cref{lem: miyake lem 4.5.12}. Furthermore, if $c_n = \mathcal O(n^\alpha)$ for some $\alpha$, then the equality above holds on $\Re(s)>\alpha + 1$. If $f$ is a cusp form, then we have this growth condition on $c_n$ due to \cref{cor: cuspform growth corollary}. We do not prove the result when $f$ is not a cusp form.
\end{proof}

The next lemma gives a necessary and sufficient condition for a modular form $f\in\mathcal M_k(N,\chi)$ to be a simultaneous eigenfunction of all of the Hecke operators $T_n$, in terms of being able to write down an Euler product for the $L$-function for $f$.
...
\end{theorem}
\begin{proof}
    Assume that (1) holds, and let $T_nf = t_nf$. By \cref{4.5.lem: miyake lem 4.5.15}, we have $c_n = t_nc_1$ for $n\geq 1$. Then if $c_1 = 0$, $f(z) = c_0$. Since $k\geq 1$, we have in this case that $f(z) = 0$, which cannot happen by assumption. Therefore $c_1\neq 0$. Obtain (2) from \cref{lem: miyake lem 4.5.15}(2), taking $P$ to be the set of all prime numbers $\mathbb P$.
...
\begin{proof}
    ...
    From \cref{prop: computing adjoints}(2), we have \[T_p^\ast = |_k[\varGamma_1(N)\big(\!\begin{smallmatrix}
        1 & 0 \\ 0 & p
...
\end{proof}
\begin{corollary}
    By the spectral theorem, the space $\mathcal S_k(\varGamma_1(N))$ has an orthogonal basis of simultaneous eigenvectors, called \textit{eigenforms}, for the Hecke operators $\cbr{\abr{n},T_n : \gcd(n,N) = 1}$.
\end{corollary}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \end{smallmatrix}\!\big)\varGamma_1(N)].
    \end{align*}
    The result follows from the fact that $T_p^\ast = |_k[\varGamma_1(N)\big(\!\begin{smallmatrix}
        1 & 0 \\ 0 & p
    \end{smallmatrix}\!\big)\varGamma_1(N)]^\ast = |_k[\varGamma_1(N)\big(\!\begin{smallmatrix}
        p & 0 \\ 0 & 1
    \end{smallmatrix}\!\big)\varGamma_1(N)]$.
...
\begin{definition}
    Denote by \textib{$\mathcal R^\ast(N)$} the polynomial ring 
...
\begin{definition}
    In what follows, denote by \textib{$\alpha_\ell$} the matrix
    \[\begin{pmatrix}
        \ell & 0 \\ 0 & 1
    \end{pmatrix}\] for a positive integer $\ell$. The action of matrices of this form on modular forms appears many times in what follows.
\end{definition}
...
\begin{lemma}\label{lem: miyake lem 4.6.1}
    ...
    \[(f\circ\alpha_\ell)(z) = f(\ell z) = d^{-k/2}(f|_k[\alpha_\ell])(z).\] If $f$ is a cusp form, then so is $f\circ\alpha_\ell$.
\end{lemma}
\begin{proof}
    ...
    \[(f|_k[\alpha_\ell])|_k[\gamma] = (f|_k[\alpha_\ell\gamma\alpha_\ell^{-1}])|_k[\alpha_\ell] = (f|_k[\big(\!\begin{smallmatrix}
        a & b\ell \\ cN & d
    \end{smallmatrix}\!\big)])|_k[\alpha_\ell] = \chi(d)f|_k[\alpha_\ell]\] as expected. The last statement is clear.
\end{proof}
In this setting, the map $\alpha_\ell$ provides a way to move between $\chi$-eigenspaces at different levels.

We saw at the end of the previous section that the Hecke operators $T_n$, for $n$ coprime to $N$, are simultaneously diagonalizable on $\mathcal S_k(\varGamma_1(N))$ and hence also on $\mathcal S_k(N,\chi)$. Unfortunately, $\mathcal S_k(N,\chi)$ does not necessarily have a basis of eigenfunctions of \textit{all} eigenfunctions $T_n$. However, $\mathcal S_k(N,\chi)$ does have such a basis when $\chi$ is a primitive character of conductor $N$. We might expect that if we exclude cusp forms of lower levels in $\mathcal S_k(N,\chi)$ (that is, the cusp forms appearing as the image of some suitable $\alpha_\ell$ in the manner described above), we may be able to simultaneously diagonalize all of the Hecke operators $T_n$. This is indeed the case, as we will see.
...
\begin{proof}
    ...
    \end{pmatrix}\begin{pmatrix}
        p & 0 \\ 0 & 1
    \end{pmatrix} : 0\leq j < p}\] form a set of coset representatives for $\varGamma_1(N)\backslash \varGamma_1(N)\big(\!\begin{smallmatrix}
        1 & 0 \\ 0 & p
...
\begin{proof}
    ...
    \end{cases}\] we have that $\varGamma^{\prime\prime} = \varGamma_0(N)$ or $\varGamma_0(N)$ is generated by $\big(\!\begin{smallmatrix}
        1 & 1 \\ 0 & 1
    \end{smallmatrix}\!\big)$ over $\varGamma^\prime$. Since $f(z+1) = f(z)$, we have that $f\in\mathcal M_k(N,\chi)$.
    ...
    Therefore, if $f \neq 0$, then $\chi(1+nN/\ell) = 1$ for any integer $n$ satisfying $\gcd(nN/\ell+1, \ell) = 1$. This implies that $\chi$ is defined modulo $N/\ell$, and that $N$ is divisible by $\ell m_\chi$. This proves (2) in the statement. Now assume that $\ell m_\chi\mid N$. Since $\varGamma_0(N/\ell)$ is generated by $\big(\!\begin{smallmatrix}
        1 & 0 \\ N/\ell & 1
    \end{smallmatrix}\!\big)$ over $\varGamma_0(N)$, we have that $f\in\mathcal M_k(N/\ell,\chi)$ as needed. Lastly, it is clear that if $f\circ\alpha_\ell$ is a cusp form, then so is $f$.
\end{proof}

\begin{lemma}\label{lem: miyake lemma 4.6.5}
    Let $f\in \mathcal M_k(N,\chi)$ have the Fourier series expansion $f(z) = \sum_{n=0}^\infty a_ne^{2\pi i n z}$. For a positive integer $L$, let $g$ be given by \[g(z) = \sum_{\substack{n\geq 0\\\gcd(n,L)=1}}a_ne^{2\pi i n z}.\]
...
\begin{proof}
    ...
    \[(T_pf)(z) = \frac{1}{p}\sum_{n=0}^\infty a_n\sum_{m=0}^{p-1}e^{2\pi i n (z+m)/p} = \sum_{n=0}^\infty a_{np}e^{2\pi i n z},\] where $T_p$ is acting at level $N^\prime$. Hence $(T_pf)(pz) = \sum_{n=0}^\infty a_{np}e^{2\pi i n pz}$, which belongs to $\mathcal M_k(N^\prime p,\chi)$ by \cref{lem: miyake lem 4.6.1}. Obtain $g\in\mathcal M_k(N^\prime p, \chi)$, given by
    \[g(z) = f(z) - (T_pf)(pz).\] The last assertion is clear.
\end{proof}

\begin{lemma}\label{lem: miyake lemma 4.6.6}
    Let $\chi$ be a Dirichlet character $\mod N$, $\ell$ a positive integer, and $p$ a prime number that does not divide $\ell$. Let $M = \ell N$. Then the following diagrams commute. 
...
\begin{thm}\label{thm: miyake thm 4.6.8}
    ...
    \begin{enumerate}[label=\textup{(\arabic*)}]
        ...
        \[f(z) = \sum_{p\mid \gcd(\ell,N/m_\chi)}f_p(pz).\] If $f$ is a cusp form, then we may choose $f_p$ to be cusp forms as well.
    \end{enumerate}
\end{thm}
\begin{proof}
    ...
    In particular, the $n$-th Fourier coefficient of $\big(h|_k[\varGamma_1(N(\ell^\prime)^2)\big(\!\begin{smallmatrix}
        1 & 0 \\ 0 & p
    \end{smallmatrix}\!\big)\varGamma_1(N(\ell^\prime)^2/p)]\big)(z)$ vanishes whenever $\gcd(n,\ell^\prime) = 1$. As a result, the $n$-th Fourier coefficient of $f(z)- f_p(pz)$ vanishes by \cref{eqn: 4.6.8 equation2}. Therefore, by the inductive hypothesis, we have 
...
\begin{definition}\label{def: eigenspace oldforms and newforms}
    Denote by \textib{$\mathcal S_k^1(N,\chi)$} the subspace of \textbf{\textit{oldforms at level $N$}} of $\mathcal S_k(N,\chi)$, which is generated by the set 
    ...
    Denote by \textib{$\mathcal S_k^0(N,\chi)$} the subspace of \textbf{\textit{newforms at level $N$}}, given by the orthogonal complement of $\mathcal S_k^1(N,\chi)$ in $\mathcal S_k(N,\chi)$ with respect to the Petersson inner product.
\end{definition}
...
In this section we focus on simultaneous eigenfunctions of the Hecke operators in the space of newforms.

\begin{lemma}\label{lem: miyake lemma 4.6.11}
    Let $f\in\mathcal S_k^0(N,\chi)$ have series expansion $\sum_{n=1}^\infty a_ne^{2\pi i n z}$. If $f$ is a common eigenfunction of the Hecke operators $T_n$ for all $n$ coprime to some integer $L$, then $a_n\neq 0$.
\end{lemma}
\begin{proof}
    Assume by way of contradiction that $a_1 = 0$. Then by \cref{lem: miyake lem 4.5.15}(1), we have that $a_n = 0$ for all $n$ coprime to $L$. Hence $f\in\mathcal S_k^1(N,\chi)$ by \cref{thm: miyake thm 4.6.8}, which is a contradiction.
\end{proof}
...
\begin{definition}
    An element $f\in\mathcal S_k^0(N,\chi)$ is a \textib{primitive form of conductor $N$} if the following conditions are satisfied:
    \begin{enumerate}[label=\textup{(\arabic*)}]
...
We finally prove the strong multiplicity one property for classical modular forms.
\begin{theorem}[Strong multiplicity one]\label{thm: sm1 miyake thm 4.6.19}
    Let $f$ be a primitive form of $\mathcal S_k^0(N,\chi)$ and let $g$ be an element of $ \mathcal S_k(M,\lambda)$, with Fourier series expansions $f(z) = \sum_{n=1}^\infty a_ne^{2\pi i nz}$ and $g(z) = \sum_{n=1}^\infty b_ne^{2\pi i n z}$. If $g$ is normalized with $b_1 = 1$, is a common eigenfunction of $\mathcal R(M)\cup \mathcal R^\ast(M)$, and $a_n = b_n$ for all $n$ coprime to some integer $L$, then $N = M$ and $f = g$.
\end{theorem}
\begin{proof}
    ...
    \[\frac{\varLambda_N(s,f)}{\varLambda_M(s,g)} = \frac{\varLambda_N(k-s,f|_k[\omega_N])}{\varLambda_M(k-s,g|_k[\omega_M])}\] by \cref{cor: miyake cor 4.3.7}. Furthermore, $T_ng = b_ng$, and since $g$ is a common eigenfunction of $T^\ast_n$ by assumption, we have that $T^\ast g = \overline{b_n}g$ and 
