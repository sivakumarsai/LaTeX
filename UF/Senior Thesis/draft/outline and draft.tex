...
\AtEndEnvironment{definition}{\null\hfill\ensuremath{\blacklozenge}}
...
\begin{proposition}
    For each Dirichlet character $\chi$ modulo $N$, define the $\chi$-\textib{eigenspace} of $\mathcal M_k(\varGamma_1(N))$ by 
...
\begin{definition}
    ...
    \[L(x,\chi) = \sum_{n=1}^\infty \frac{\chi(n)}{n^s} = \prod_{p\in\mathbb P}\frac{1}{1-\chi(p)p^{-s}},\quad \Re(s)>1.\] Here $\mathbb P$ denotes the set of prime numbers and the second equality above is obtained by using geometric series and the fundamental theorem of arithmetic.
\end{definition}
