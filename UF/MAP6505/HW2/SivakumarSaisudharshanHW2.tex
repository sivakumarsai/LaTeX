...
\begin{enumerate}
    ...
    \begin{align*}
        ...
        &\eq{6} \big(\cbr{\nabla f},\varphi\big) - \int_{\partial\Omega\cap B_R} n_x[f]_{\partial\Omega}(x) \varphi(x)\dd[N]x\\
        &\eq{7} \big(\cbr{\nabla f},\varphi\big) - \big(n_x[f]_{\partial\Omega}\delta_{\partial\Omega},\varphi\big)
    \end{align*} where \begin{enumerate}
        \item[(1)] is by definition of distributional derivatives,
        ...
        \item[(6)] is by additivity, definition of the jump discontinuity of $f$, the fact that $\cbr{\nabla f}$ is locally integrable and hence defines a regular distribution, and 
        \item[(7)] is by definition of the surface delta distribution (by the definition in the text, we require $n_x[f]_{\partial \Omega}(x)$ to be continuous, which is true since $[f]_{\partial \Omega}(x)$ is continuous by hypothesis).
    \end{enumerate}
    Hence the distributional equality $\nabla f = \cbr{\nabla f} - n_x[f]_{\partial\Omega}\delta_{\partial\Omega}$ holds. By inspecting components of the vectors in this equality, the distributional equality $\pdv{x_j}f = \cbr{\pdv{x_j}f}-(n_x)_j[f]_{\partial\Omega}\delta_{\partial\Omega}$ holds for each $j$.
