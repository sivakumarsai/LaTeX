...
Now we should go in the other direction. Given a weakly modular meromorphic function $f\colon\mathbb H\to\mathbb C$ of weight $2k$, we should try to obtain a meromorphic differential on $X(N)$ of degree $k$. For any point $w$ of $X(N)$, pick an element $z\in\mathbb H$ belonging to $\pi^{-1}(w)$ and an open neighborhood $U$ of $z$ for which translates of $U$ by the action of $\gamma\in \Gamma(N)$ intersect $U$ if and only if $\gamma$ stabilizes $z$ (this accounts for elliptic points). Let $V = \pi(U)$ be a chart around $p$ and $\psi\colon V\to W$ be the chart map of $X(N)$ for $V$ with $W$ an open subset of $\mathbb C$ (these form a holomorphic atlas for each point).

Now choose $\alpha\colon W\to \mathbb C$ for each $p$ so that locally $f$ restricted to $U$ is equal to $\alpha\circ\psi\circ\pi$ restricted to $U$ (this will not appear until later). When $z$ is not a cusp, define $g\colon U\to \mathbb C$ by $g(s) = f|_U(s) (\dv{\psi\circ\pi}{s}(s))^{-k}$. Due to our choice of $\psi$ (from $U$), for $\gamma$ in the stabilizer of $z$, $\psi\circ\pi(\gamma s) = \psi\circ\pi(s)$ for $s\in U$. So $g(\gamma s) = f(\gamma s)(\dv{\psi\circ\pi(\gamma s)}{\gamma s})^{-k} = j(\gamma,s)^{2k}f(s)(\dv{\psi\circ\pi(s)}{s}j(\gamma,s)^2)^{-k} = g(s)$. So $g$ is really invariant under the action of $\gamma$ in the stabilizer of $z$. So there is a meromorphic function $h\colon W\to \mathbb C$ for which $g = h\circ\psi\circ \pi$ on $U$.

Now consider another point $q$ that is not a cusp, and repeat the constructions above for some point $\tilde z$ in the preimage of $q$ under $\pi$ (everything marked with a tilde). Then $\tilde h\circ \tilde \psi\circ \pi = f|_{\tilde U}(\dv{\tilde\psi\circ\pi}{z})^{-k}$. Since $\alpha = \tilde\alpha\circ \psi_{pq}$ (the transition function between the open sets coming from $p,q$), $h = (\tilde h\circ \psi_{pq})(\dv{\psi}{\tilde\psi})^{-k}$. This will allow us to glue together the $h,\tilde h$ (and the other similar functions obtained from other non cusp points) together.

Now let $z$ be a cusp. Repeat the same constructions as above but define $g$ on the punctured neighborhood $U\setminus \{z\}$. Obtain $h$ as well and define it on $W\setminus \{\psi(z)\}$, so that $g = h \circ\psi\circ \pi$ on $U\setminus\{z\}$. For sake of computation we would like $z$ to be $\infty$, so choose $\sigma\in \Gamma$ so that $\gamma z =\infty$ and $\pm I\sigma\Stab(z)\sigma^{-1}$ is generated by $\pm I \bigl(\!\begin{smallmatrix}
    1 & n \\ 0 & 1
\end{smallmatrix}\!\bigr)$ (this $n>0$ is some kind of period). Define $\phi\colon V\to W$ by $\phi\circ \pi = \exp(2\pi i \sigma(z)/n)$ and the calculation $g(s) = (2\pi i/n)^{-k}f(s)(\dv{\sigma(s)}{s})^{-k}(\phi\circ\pi(s))^{-n} = (2\pi i/n)^{-k}f(s)j(\gamma,s)^{2k}(\phi\circ\pi(s))^{-n}$. This shows that $g$ is meromorphic at $z$ (since the right hand side is meromorphic at $\infty$ and is just a modified version of $f$ where it thinks the cusp $p$ is at $\infty$). The gluing of the $h$ coming from this $g$ with the other $h$ from other points is a similar calculation as before. Hence the collection of functions $h$ defined at all these points constitutes a meromorphic differential of degree $k$.
...
\[\dim(M_0(\Gamma)) = 1,\quad \dim(M_2(\Gamma)) = 0, \quad\dim(M_{4,6,...,10}(\Gamma)) = 1, \quad\dim(M_{12}(\Gamma)) = 2,\]
\[\text{and}\quad \dim(M^0_{12}(\Gamma)) = 1.\]
At this point it is clear that $G_4$ spans $M_4(\Gamma)$ and that $G_6$ spans $M_6(\Gamma)$. So then $G_4^2$ spans $M_8(\Gamma)$ and $G_4G_6$ spans $M_{10}(\Gamma)$. Similarly $\Delta$ spans $M^0_{12}(\Gamma)$.

For $f\in M_{2k}(\Gamma)$ with $k\geq 6$, choose integers $a,b$ so that $4a+6b= 2k$. Then $G_4^aG_6^b$ is a weight $2k$ modular form and is specifically not a cusp form since the Eisenstein series don't vanish at $\infty$. There exists a constant $c$ for which $f-cG_4^aG_6^b$ vanishes at infinity (the constant term is killed), and since the weight of this form is at least $12$, we can divide by the cusp form $\Delta$ to find $(f-cG_4^aG_6^b)/\Delta\in M_{2k-12}$. By induction this quotient is a multiple of $G_4^sG_6^t$ where $4s+6t = 2k-12$. And since $\Delta$ itself is a linear combination of $G_4^3$ and $G_6^2$, it follows that $f$ is as well.

It remains to see that powers of $G_4$ and powers of $G_6$ are linearly independent. If they were not, then it is possible (by starting with a linear dependence between the powers) to arrange for $G_4^3/G_6^2$ to satisfy a polynomial equation; from which it follows that $G_4^3/G_6^2$ is constant. Evaluating at $z = \exp(2\pi i/3)$, $G_4$ is zero but $G_6$ is nonzero so the constant must be zero. But $G_4^3/G_6^2$ is not zero everywhere, a contradiction. Hence $\bigoplus_{k\in\mathbb Z} M_{2k}(\Gamma)$ is the polynomial algebra $\mathbb C[G_4, G_6]$.

Now let $f$ be a cusp form of weight $2k$ (which will be at least $12$). Then $f/\Delta$ is still holomorphic since $f$ was a cusp form, and hence the quotient is a modular form of weight $2k-12$, which belongs to $\mathbb C[G_4,G_6]$. Thus $f\in \Delta\mathbb C[G_4,G_6]$. On the other hand, given an element of $\Delta\mathbb C[G_4,G_6]$, it is a cusp form since it will have zero constant term near infinity (due to multiplication with $\Delta$ which has no constant term at infinity). Hence the space of all cusp forms is the ideal generated by $\Delta$ in the polynomial algebra $\mathbb C[G_4,G_6]$.
...
Lecture notes for Lecture 7 of 18.786 Number theory II by Andrew V. Sutherland (\href{https://math.mit.edu/classes/18.786/2024/LectureNotes7.pdf}{link to PDF})
