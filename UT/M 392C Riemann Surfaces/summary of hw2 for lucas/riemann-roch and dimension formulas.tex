...
Due to the elliptic points coming from $\Gamma$, meromorphic modular functions of weight $2k$ may have orders that are fractional and not just integers. So to start we will consider the $\mathbb Q$-vector space of divisors $\Div_{\mathbb Q}(X)$ on $X$ and carefully reduce calculations to ones involving integer coefficients so that we can use the Riemann-Roch theorem. Henceforth we also suppress the argument $(z)$ of functions.
