...
\end{align*}
on $U_\alpha\cap U_\beta$ (and the same can be said about pulling back along the transition function in the other direction). There is a notion of equivalence of meromorphic differentials which we do not record here. Denote the space of meromorphic differentials on $R$
...
We will do part of the calculation. Let $f(z)$ be an automorphic form of weight $2k$ on $\mathbb H^\ast$. We will describe the component of the meromorphic differential $\omega = \{\phi_\alpha\}$ of degree $k$ it defines on $X(\Gamma)$ around a point $a = \pi(z_0)\in X(\Gamma)$ for $z_0\in \mathbb H$. That is, we will define a meromorphic function $\phi_a$ that $\omega$ looks like around $a$. There is a neighborhood $U_{z_0}$ of $z_0$ in $\mathbb H$ and a chart $(V_\alpha = \pi(U_{z_0}), V_a\xrightarrow{\varphi_a}U_a)$ containing $a$. Consider the function 
\[g(z)= f(z)\biggl(\dv{(\varphi_a\circ\pi)(z)}{z}\biggr)^{-k}\]
defined on $U_{z_0}$. In principle we could define $g(z)$ on all $\Gamma$-translates of $U_{z_0}$, but since $\pi$ is the quotient map $\mathbb H^\ast\to\Gamma\backslash \mathbb H^\ast$, it is enough to look at $U_{z_0}$. We would like to define a function 
