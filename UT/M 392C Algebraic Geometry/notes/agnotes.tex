...
\begin{document}
M 392C Algebraic Geometry, Fall 2024: Dr. Chiara Damiolini's notes, \TeX ed by Sai Sivakumar. Text references: \textit{Algebraic Geometry I: Schemes} by G\"ortz and Wedhorn, and \textit{Algebraic Geometry} by Hartshorne.

\sai{ask jemma for her notes at some point for inspiring art}
\sai{italicize nice words/phrases throughout tastefully}
\sai{fix enumerates? textups too in enumerates}\sai{check for K-alg vs k-alg for categories and morphisms}
\section{08/26}
...
\begin{proposition}[cf. \cite{gw}*{Proposition 2.20}]
    ...
    \end{equation}
    We view $\mathcal B$ as a subcategory of $\Ouv(X)$.
\end{proposition}
\sai{proof}
...
\begin{theorem}\label{thm: structure sheaf}
    ...
    \end{enumerate}
\end{theorem}
As a reminder, $R_{\mathfrak p}$, the localization of a ring $R$ at a prime ideal $\mathfrak p$, is the localization of $R$ at the multiplicative set $S = R\setminus \mathfrak p$. So 
\begin{equation}
...
\sai{note from sai to include in the notes: defining stalks a lecture early because the following discussion about the sheaf from earlier makes no sense without this.}

We can think of the sheaf in \cref{thm: structure sheaf} as a ``space of functions, but with a moving target''. To explain, recall that the localization of $R$ at a prime ideal $\mathfrak p$, $R_{\mathfrak p}$, is a local ring with maximal ideal $\mathfrak p$. Let $U\in\Ouv(X)$, $\mathfrak p\in U$, and $f\in \mathscr O_XU$. Consider sending $f$ to its germ $f$ (the equivalence class of $(U,f)$) in the stalk $\mathscr O_{X,\mathfrak p} = R_{\mathfrak p}$.
Then the map $f \mapsto f(\mathfrak p)\colon \mathscr O_{X,\mathfrak p}\to \mathscr O_{X,\mathfrak p}/\mathfrak m_{\mathfrak p}$ 
% The moving target refers to the fact that $f$ sends $\mathfrak q$ to an element of $R_{\mathfrak q}\subset \coprod_{\mathfrak p\in U}R_{\mathfrak p}$, so in a sense, the codomain of $f$ is shifting. 
\sai{For example, if $R$ is a domain, we can choose the ``target'' to be $\Frac R$. What? (ask Chiara)} In a way, we can view the elements of $R$ as functions on $\Spec R$.
