...
\begin{enumerate}
  ...
  % Indeed, restricting the map $J$ to $\im F$, we obtain an isometric embedding of $\im F$ in $X^{\ast\ast\ast\ast}$, in particular in $(\im F)^\ast$. Let $E\colon X^{\ast}\to X^{\ast\ast\ast}$ be the canonical isomorphism, and consider its adjoint $E^\ast\colon X^{\ast\ast\ast\ast}\to X^{\ast\ast}$, which by a similar argument as before is a linear isometry. The map $E^\ast$ restricted to $(\im F)^{\ast\ast}$ is a right inverse to $J$ restricted to $\im F$: For any functional $f\in (\im F)^{\ast\ast}$ and $g\in X^{\ast\ast\ast}$, $(JE^\ast )f(g) = J_fE_g=f(g)$.
