...
Let $G = \abr{x}$ be a cyclic group of order $n$. The dual object $\widehat{G}$ is given by the $n$-th roots of unity in $\mathbb C$: \sai{draw picture}

In this case, $\Spec$ is taking the maximal ideals of rings. With $\mathbb C[G]\cong \mathbb C[x]/(x^n-1)$, it is equivalent to describe the elements in $\Hom_\Ring(\mathbb C[x]/(x^n-1),\mathbb C)$, which are characterized by each of the $n$-th roots of unity.

Given an irreducible representation $V$ of $G$ (i.e. a simple $\mathbb C[G]$-module), we can try to form a sheaf out of $V$ on $\widehat G$. Since $V$ is irreducible (moreover, it's even one-dimensional), this sheaf has to be concentrated at only one of the points of $\widehat{G}$. The point that $V$ is concentrated at is the root of unity $\zeta$ for which the action of $x$ on $V$ is given by multiplication by $\zeta$. So we can also think of $\widehat G$ as the collection of irreducible representations of $G$.
