...
where $\Fun(G)$, $\Fun(\widehat G)$ are function spaces on $G,\widehat G$ and are given convolution and pointwise multiplication, respectively (we will describe the action of $G$ on each of these spaces next time). This isomorphism has some symmetry which is part of the statement of Pontryagin duality.
...
If $V$ is a representation of $G$, another representation of $G$ we can form is the dual space $V^\ast\coloneqq \Hom_{\mathbb C[G]}(V,\mathbb C)$, where the action of $G$ on $\mathbb C$ is the trivial action. The action of $G$ on $V^\ast$ is $(gf)(v) = f(g^{-1}v)$. We may not be able to  construct dual modules for an arbitrary $\mathbb C$-algebra $A$ and $A$-module $V$, since we need some way to invert the action of $A$ when ``passing the action inside the function''. This is related to one other property that Hopf algebras have, which is their antipode or coinverse map. In the case of the group algebra $\mathbb C[G]$, the coinverse map is $g\mapsto g^{-1}$ and this actually gives an isomorphism of $\mathbb C[G]$ and its opposite ring $\mathbb C[G]^{\op}$ (the ring where $g\cdot_{\op}h = gh$).

If $A$ is a Hopf algebra and $V,W$ are $A$-modules it is possible to define actions of $A$ on $V\otimes W$ and on $V^\ast$ in a manner analogous to the above by using the comultiplication and antipode maps on $A$, respectively.

If we want to think of the tensor product as giving a monoid structure to $\Rep_{\mathbb C}(G)$ (the category of dimensional representations of $G$), we might wonder if there is a way to obtain inverses in this category. So far, being able to take tensor products and duals of representations make $\Rep_{\mathbb C}(G)$ a rigid symmetric monoidal category; see the \href{https://ncatlab.org/nlab/show/rigid+monoidal+category}{nLab}. Another question is when the irreps of $G$ form a monoid or even group; and in the case that $G$ is not Abelian it is possible that the tensor product of irreps need not be an irrep...
...
where $\chi_{\hat g}$ is the character corresponding to $\hat g\in \widehat G$ (here $\widehat G$ is thought of as a set of points; we will regularly switch between the equivalent forms for $\widehat G$). Observe that $\chi$ is multiplicative in each component; that is,
...
\[\chi_{gh}(\hat g) = \chi(gh,\hat g) = \chi(g,\hat g)\chi(h,\hat g) = \chi_g(\hat g)\chi_h(\hat g)\quad \text{Thus $\chi_{gh} = \chi_g\chi_h$ (pointwise multiplication)}\]
