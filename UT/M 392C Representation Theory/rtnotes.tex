...
\begin{enumerate}
    ...
    From the action of $G$ on $X$, function spaces on $X$ inherit a natural linear action of $G$; one is given by $gf(x) = f(xg^{-1})$ (in this case $G$ acts on the right of $X$ to obtain a left action on functions). These representations are typically very decomposable or reducible.
...
\begin{enumerate}
  ...
  \[\text{commutative rings}\xlongleftrightarrow{\cong} \text{geometry}\]
...
An $R$-module $M$ has the decomposition $M = \oplus_i e_i M$, so we obtain a sheaf on $X$ where at each point of $X$ we can imagine the corresponding module $e_iM$ lying on it:
\begin{align*}
...
A representation $V$ of $G$ is given by a group homomorphism $G\xrightarrow{\rho}\Aut(V)$. Since $\Aut(V)$ is contained in $\End(V)$, a $k$-algebra, there is an object $k[G]$ called the group algebra for which the homomorphism $\rho$ factors through $k[G]$; that is, the diagram 
...
\[k[G] = \{\sum_{g\in G}c_gg\mid c_g\in k, \text{ all but finitely many $c_k$ are zero}\}.\]
