...
The action of $G$ on the irrep $V$ is given by scalar multiplication by $\chi_V(g)\in \mathbb C^\times$ since $V$ is one-dimensional. More importantly, the assignment $g\mapsto\chi_V(g)$ is a group homomorphism $G\to \mathbb C^\times$, called a character of $G$.
...
\end{tikzcd}\]
Every map $\overline f\in \Hom_{\mathbb C\alg}(\mathbb C[G],\mathbb C)$ corresponds uniquely to a map $f\in \Hom_{\Group}(G,\mathbb C^\times)$. So in general we can also think of $\widehat G$ as the collection of irreps of $G$.
